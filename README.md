...
- `src/client.c`: interactive command-line client
- `src/server.c`: multi-client TCP server
- `src/common.c`, `src/common.h`: shared socket, message, and file-transfer helpers
- `src/msg_struct.h`: message protocol definitions
- `Makefile`: builds the client and server

Runtime folders are created by `make` and excluded from Git.
...
At the login prompt, enter `/login robin demo-password`. Type `/listcmd` in the client to see the commands supported by the running server. Connect multiple clients to the same server for a local demonstration.
